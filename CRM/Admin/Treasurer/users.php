<?php
  // users.php - Add this at the VERY TOP of the file
  session_start();

  // Include the functions file
  require_once dirname(__DIR__, 2) . '/System/Config/Backend/Controllers/userRoles.functions.php';

  // Fetch user roles data
  $rolesResult = fetchUserRolesData();

  // Get user roles count (excluding IT)
  $userRolesCount = countUserRolesExcludingIT();

  // Convert PHP data to JSON for JavaScript - FIXED VERSION
  $userRolesData = $rolesResult['success'] ? $rolesResult['data'] : [];
  $userRolesJson = json_encode($userRolesData, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);

  // Debug PHP data
  error_log("PHP fetched " . count($userRolesData) . " roles");
  if (count($userRolesData) > 0) {
      error_log("First role: " . print_r($userRolesData[0], true));
  }
?>
...
<html lang="en">
  ...
  <head>
    ...
    <title>Users | KFBCL - Mircofinance</title>
  ...
  <body x-data="{ page: 'users', 'loaded': true, 'darkMode': false, 'stickyMenu': false, 'sidebarToggle': false, 'scrollTop': false, 'editUserRoleModal': false, 'newUserRoleModal': false,  'newUserModal': false, 'editUserModal': false, 'isProfileAddressModal': false }">
    ...
    <!-- ===== Sidebar End ===== -->
      ...
      <div class="relative flex flex-1 flex-col overflow-x-hidden overflow-y-auto">
        ...
        <!-- Table loaders -->
        <!-- Debug script - add this right after the opening <body> tag -->
        <script>
          console.log('=== DEBUG START ===');
          console.log('userRolesFromPHP exists:', typeof window.userRolesFromPHP !== 'undefined');
          console.log('userRolesFromPHP type:', typeof window.userRolesFromPHP);
          console.log('userRolesFromPHP value:', window.userRolesFromPHP);
          console.log('Is array?', Array.isArray(window.userRolesFromPHP));
          console.log('Length:', window.userRolesFromPHP ? window.userRolesFromPHP.length : 0);

          // Try to see the raw JSON
          console.log('Raw PHP JSON:', '<?php echo addslashes($userRolesJson); ?>');

          // Check if there are any roles
          if (window.userRolesFromPHP && window.userRolesFromPHP.length > 0) {
            console.log('First role:', window.userRolesFromPHP[0]);
            console.log('Keys in first role:', Object.keys(window.userRolesFromPHP[0]));
          }
          ...
          <div class="mx-auto max-w-(--breakpoint-2xl) p-4 pb-20 md:p-6 md:pb-6">
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3"
                   x-data="usersTableFull()"
                   x-init="$nextTick(() => {
                 // This ensures Alpine has initialized everything
                 console.log('Metrics loaded, store available:', Alpine.store('userModal'));
               })">
                ...
                <article class="flex items-center gap-5 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
                  ...
                  <div>
                    <h3 class="text-2xl font-semibold text-gray-800 dark:text-white/90" x-text="endEntry">
                      7 <!-- Hardcoded for now, will update via JS -->
                    </h3>
                ...
                <article class="flex items-center gap-5 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
                  ...
                  <div>
                    <h3 class="text-2xl font-semibold text-gray-800 dark:text-white/90">
                      3 <!-- Hardcoded for now, will update via JS -->
                    </h3>
                ...
                <article class="flex items-center gap-5 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/3">
                  ...
                  <div>
                    <h3 class="text-2xl font-semibold text-gray-800 dark:text-white/90">
                      <?php echo htmlspecialchars($userRolesCount); ?>
                    </h3>
                    <p class="flex items-center gap-3 text-gray-500 dark:text-gray-400">
                      User Roles
                    </p>
                                    ...
                                    <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-900">
                                      ...
                                      <td class="p-4 whitespace-nowrap">
                                        <p class="text-sm text-gray-700 dark:text-gray-400 truncate max-w-[200px]"
                                          x-text="row.lastLogin"
                                          :title="row.lastLogin"></p>
                                      </td>
                        ...
                        <div>
                              ...
                              <div x-data="userRolesTable">
                                <div class="custom-scrollbar overflow-x-auto">
                                  ...
                                  <!-- Your existing table - wrap it with this -->
                                        ...
                                        <template x-for="role in paginatedRoles" :key="role.UserRoleID">
                                          <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-900">
                                            ...
                                            <td class="p-4 whitespace-nowrap">
                                              <div class="flex items-center col-span-2">
                                                <p class="text-xs font-sm text-gray-700 dark:text-gray-400" 
                                                  x-text="role.CreatedOn ? formatDate(role.CreatedOn) : 'N/A'">
                                                </p>
                                              </div>
                                            </td>
                                            <!-- #Created By -->
                                              ...
                                              <ul class="divide-y divide-gray-100 dark:divide-gray-800">
                                                ...
                                                <li class="flex items-center gap-5 py-2.5">
                                                  ...
                                                  <span class="w-1/2 text-sm font-medium text-gray-700 sm:w-2/3 dark:text-gray-400">
                                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                                      <!-- Create -->
                                                      <template x-if="role.BodabodaCreate">
                                                        <span class="text-green-600">Create</span>
                                                      </template>
                                                      <template x-if="role.BodabodaCreate && (role.BodabodaRead || role.BodabodaUpdate || role.BodabodaDelete)">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Read -->
                                                      <template x-if="role.BodabodaRead">
                                                        <span class="text-blue-600">Read</span>
                                                      </template>
                                                      <template x-if="role.BodabodaRead && (role.BodabodaUpdate || role.BodabodaDelete)">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Update -->
                                                      <template x-if="role.BodabodaUpdate">
                                                        <span class="text-yellow-600">Update</span>
                                                      </template>
                                                      <template x-if="role.BodabodaUpdate && role.BodabodaDelete">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Delete -->
                                                      <template x-if="role.BodabodaDelete">
                                                ...
                                                <li class="flex items-center gap-5 py-2.5">
                                                  ...
                                                  <span class="w-1/2 text-sm font-medium text-gray-700 sm:w-2/3 dark:text-gray-400">
                                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                                      <!-- Create -->
                                                      <template x-if="role.LoansCreate">
                                                        <span class="text-green-600">Create</span>
                                                      </template>
                                                      <template x-if="role.LoansCreate && (role.LoansRead || role.LoansUpdate || role.LoansDelete)">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Read -->
                                                      <template x-if="role.LoansRead">
                                                        <span class="text-blue-600">Read</span>
                                                      </template>
                                                      <template x-if="role.LoansRead && (role.LoansUpdate || role.LoansDelete)">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Update -->
                                                      <template x-if="role.LoansUpdate">
                                                        <span class="text-yellow-600">Update</span>
                                                      </template>
                                                      <template x-if="role.LoansUpdate && role.LoansDelete">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Delete -->
                                                      <template x-if="role.LoansDelete">
                                                        <span class="text-red-600">Delete</span>
                                                      </template>
                                                ...
                                                <li class="flex items-center gap-5 py-2.5">
                                                  ...
                                                  <span class="w-1/2 text-sm font-medium text-gray-700 sm:w-2/3 dark:text-gray-400">
                                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                                      <!-- Create -->
                                                      <template x-if="role.LandsCreate">
                                                        <span class="text-green-600">Create</span>
                                                      </template>
                                                      <template x-if="role.LandsCreate && (role.LandsRead || role.LandsUpdate || role.LandsDelete)">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Read -->
                                                      <template x-if="role.LandsRead">
                                                        <span class="text-blue-600">Read</span>
                                                      </template>
                                                      <template x-if="role.LandsRead && (role.LandsUpdate || role.LandsDelete)">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Update -->
                                                      <template x-if="role.LandsUpdate">
                                                        <span class="text-yellow-600">Update</span>
                                                      </template>
                                                      <template x-if="role.LandsUpdate && role.LandsDelete">
                                                        <span class="text-gray-400"> | </span>
                                                      </template>
                                                      
                                                      <!-- Delete -->
                                                      <template x-if="role.LandsDelete">
                                                        <span class="text-red-600">Delete</span>
                                                      </template>
                                            ...
                                            <td class="p-4 whitespace-nowrap">
                                              <div class="flex items-center col-span-2">
                                                ...
                                                <!-- Inactive -->
                                                <template x-if="role.Status === 'Inactive'">
                                                ...
                                                <!-- Removed/Deleted -->
                                                <template x-if="role.Status === 'Removed' || role.Status === 'Deleted'">
                                                  <p class="bg-danger-50 text-theme-xs text-danger-700 dark:bg-danger-500/15 dark:text-danger-500 rounded-full px-2 py-0.5 font-medium">
                                                    <span x-text="role.Status"></span>
                                                  </p>
                                                </template>
                                                <!-- Any other status -->
                                                <template x-if="role.Status && !['Active', 'Inactive', 'Removed', 'Deleted'].includes(role.Status)">
            ...
            <div class="-mx-2.5 flex flex-wrap gap-y-5">
              ...
              <div class="w-full px-2.5 xl:w-1/2">
                ...
                <div class="relative z-20 bg-transparent">
                  <select x-model="newUserForm.gender"
              ...
              <div class="w-full px-2.5 xl:w-1/2">
                ...
                <div class="relative">
                  <input type="text"
                         x-model="newUserForm.dateOfBirth"
              ...
              <div class="w-full px-2.5 xl:w-1/2">
                ...
                <input type="text"
                       x-model="newUserForm.nationalId"
    ...
    <div @click="$store.userModal.editUserModal = false" class="modal-close-btn fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px]"></div>
      ...
      <div @click.outside="editUserModal = false" class="relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        ...
        <div class="px-2 pr-14">
          <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
            Edit User Details
            <span x-show="currentUser" x-text="' - ' + currentUser.firstName + ' ' + currentUser.lastName" class="text-brand-500"></span>
          </h4>
          <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
          ...
          <form>
            <div class="-mx-2.5 flex flex-wrap gap-y-5">
              ...
              <div class="w-full px-2.5 xl:w-1/2">
                ...
                <div class="relative z-20 bg-transparent">
                  <select x-model="$store.userModal.editForm.gender"
                          class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-3 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                    <option value="">Select Gender</option>
                    <option value="Male">Male</option>
                    <option value="Female">Female</option>
              ...
              <div class="w-full px-2.5 xl:w-1/2">
                <h3 class="text-normal font-semibold text-gray-500 dark:text-white/90 mb-2">User Role</h3>
                <div class="relative z-20 bg-transparent">
                  <select x-model="$store.userModal.editForm.userRole"
                          class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-3 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                    <option value="">Assign Role</option>
                    <option value="Chairman">Chairman</option>
                    <option value="Secretary General">Secretary General</option>
              ...
              <div class="w-full px-2.5 xl:w-1/2">
                ...
                <div class="relative z-20 bg-transparent">
                  <select x-model="$store.userModal.editForm.userStatus"
                          class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-3 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
    ...
    <div x-show="newUserRoleModal" class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto z-99999">
      ...
      <div @click.outside="newUserRoleModal = false"
        ...
        <form method="POST" action="../../System/Config/Backend/Controllers/userRoles.Controller.php">
          ...
          <!-- Error messages Here -->
           <?php
            ...
            <div class="rounded-xl border border-green-500 bg-green-50 p-4 dark:border-green-500/30 dark:bg-green-500/15">
              <div class="flex items-start gap-3">
                <div class="-mt-0.5 text-green-500">
                  <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M20.7071 5.29289C21.0976 5.68342 21.0976 6.31658 20.7071 6.70711L9.70711 17.7071C9.31658 18.0976 8.68342 18.0976 8.29289 17.7071L3.29289 12.7071C2.90237 12.3166 2.90237 11.6834 3.29289 11.2929C3.68342 10.9024 4.31658 10.9024 4.70711 11.2929L9 15.5858L19.2929 5.29289C19.6834 4.90237 20.3166 4.90237 20.7071 5.29289Z" fill="#12B76A"></path>
                  </svg>
                </div>
                <div>
                  <h4 class="mb-1 text-sm font-semibold text-gray-800 dark:text-white/90">
            ...
            <?php if (isset($_GET['error'])): 
              $errorMessages = [
                'role_failed' => 'Failed to create role.',
                'privileges_failed' => 'Failed to assign privileges.',
                'database_error' => 'Database error occurred.'
              ];
              $message = $errorMessages[$_GET['error']] ?? 'An error occurred.';
            ?>
          ...
          <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
            ...
            <div class="col-span-1">
              ...
              <input type="text" name="role_name" placeholder="Enter new user role name" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800">
            </div>
            ...
            <div class="col-span-1">
              ...
              <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                <!-- ADDED: name="status" -->
                <select name="status" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" :class="isOptionSelected &amp;&amp; 'text-gray-800 dark:text-white/90'" @change="isOptionSelected = true">
    ...
    <div x-show="$store.userRoleModal.editUserRoleModal" 
      ...
      <div @click.outside="$store.userRoleModal.editUserRoleModal = false" class="relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        ...
        <div class="px-2 pr-14">
          <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
            Edit <span x-text="$store.userRoleModal.currentRole?.userRole || ''" class="text-brand-500"></span> Role
          </h4>
          <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
            Review user role and privileges.
          </p>
        </div>
        <!-- Edit User role form -->
        <form @submit.prevent="updateRoleSubmit" class="">
          <h4 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90">
            #ID: <span x-text="$store.userRoleModal.currentRole?.UserRoleID || ''" class="text-brand-500"></span>
          </h4>
            ...
            <div class="col-span-1">
              ...
              <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                <select x-model="$store.userRoleModal.editForm.status"
                  ...
                  <option value="Removed" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                    Removed
                  </option>
                </select>
                <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                ...
                <table class="w-full">
                  ...
                  <tbody class="py-3 divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr>
                      ...
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.BodabodaCreate || false"
                              @change="$store.userRoleModal.currentRole.BodabodaCreate = $event.target.checked"
                      ...
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.BodabodaRead || false"
                              @change="$store.userRoleModal.currentRole.BodabodaRead = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-read-boda">
                      </td>
                      <!-- Update -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.BodabodaUpdate || false"
                              @change="$store.userRoleModal.currentRole.BodabodaUpdate = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-update-boda">
                      </td>
                      <!-- Delete -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.BodabodaDelete || false"
                              @change="$store.userRoleModal.currentRole.BodabodaDelete = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-delete-boda">
                      </td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LoansCreate || false"
                              @change="$store.userRoleModal.currentRole.LoansCreate = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-create-loans">
                      </td>
                      <!-- Read -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LoansRead || false"
                              @change="$store.userRoleModal.currentRole.LoansRead = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-read-loans">
                      </td>
                      <!-- Update -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LoansUpdate || false"
                              @change="$store.userRoleModal.currentRole.LoansUpdate = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-update-loans">
                      </td>
                      <!-- Delete -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LoansDelete || false"
                              @change="$store.userRoleModal.currentRole.LoansDelete = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-delete-loans">
                      </td>
                    </tr>
                    ...
                    <tr>
                      ...
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LandsCreate || false"
                              @change="$store.userRoleModal.currentRole.LandsCreate = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-create-lands">
                      </td>
                      <!-- Read -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LandsRead || false"
                              @change="$store.userRoleModal.currentRole.LandsRead = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-read-lands">
                      </td>
                      <!-- Update -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LandsUpdate || false"
                              @change="$store.userRoleModal.currentRole.LandsUpdate = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-update-lands">
                      </td>
                      <!-- Delete -->
                      <td class="px-6 py-3 whitespace-nowrap first:pl-0">
                        <input type="checkbox" 
                              :checked="$store.userRoleModal.currentRole?.LandsDelete || false"
                              @change="$store.userRoleModal.currentRole.LandsDelete = $event.target.checked"
                              class="h-5 w-5 rounded border-gray-300 text-brand-500" 
                              id="e-delete-lands">
                      </td>
                    </tr>
    ...
    <!-- Scripts -->
    <!-- Add this right before your Alpine.js script tag -->
    <script>
      window.GlobalUserRolesData = <?php echo $userRolesJson; ?>;
      window.GlobalUserRolesCount = <?php echo $userRolesCount; ?>;

      console.log('GlobalUserRolesData loaded:', window.GlobalUserRolesData);
      console.log('Count:', window.GlobalUserRolesCount);
    </script>

    <script>
      // Check if we need to refresh the data
      if (window.location.search.includes('refresh=true')) {
        // You can either reload the page or make an AJAX call to refresh data
        setTimeout(() => {
          ...
          console.log('New role created, refresh data if needed');
        }, 100);
      }
    </script>
    ...
    <script>
      document.addEventListener('alpine:init', () => {
        ...
        // 1. First, create simple variables for stats
        let userStats = {
          totalUsers: 0,
          activeUsers: 0
        };

        // 2. Create the global store for user modal
        Alpine.store('userModal', {
          ...
          updateUser() {
            // Get the table component and call its updateUser method
            ...
            if (tableComponent && tableComponent.updateUser) {
              ...
              // Update stats after user is updated
              this.updateStats(tableComponent.filteredRows);
            }
          },

          // Simple function to update stats
          updateStats(rows) {
            if (!rows) return;

            userStats.totalUsers = rows.length;
            // Count active users
            userStats.activeUsers = rows.filter(user => user.userStatus === 'Active').length;
          },

          // Direct getters for stats
          get totalUsers() {
        ...
        // 3. Create the global store for user role modal
        Alpine.store('userRoleModal', {
          ...
          updateRole() {
            // Get the user roles table component and call its updateRole method
        ...
        Alpine.data('usersTableFull', () => ({
          allRows: [
            {
              ...
              userRole: "Chairman",
              userStatus: "Active",
              joinedOn: "15-Dec-24 10:30",
              lastLogin: "06-Jan-25 13:12"
            },
            {
              id: 2,
              userId: "usr202402",
              ...
              nationalId: "23456789",
              dob: "1990-08-22",
              userRole: "Secretary General",
              userStatus: "Active",
              joinedOn: "16-Dec-24 14:00",
              lastLogin: "05-Jan-25 09:45"
            },
            {
              id: 3,
              userId: "usr202403",
              ...
              joinedOn: "17-Dec-24 11:00",
              lastLogin: "04-Jan-25 16:30"
            },
            {
              id: 4,
              userId: "usr202404",
              ...
              nationalId: "45678901",
              dob: "1992-11-05",
              userRole: "Stage Manager",
              userStatus: "Active",
              joinedOn: "18-Dec-24 15:30",
              lastLogin: "03-Jan-25 11:20"
            },
            {
              id: 5,
              userId: "usr202405",
              ...
              userRole: "Secretary",
              userStatus: "Pending",
              joinedOn: "19-Dec-24 09:00",
              lastLogin: "02-Jan-25 14:15"
            },
            {
              id: 6,
              userId: "usr202406",
              ...
              dob: "1995-02-28",
              userRole: "Supervisor",
              userStatus: "De-Activated",
              joinedOn: "20-Dec-24 13:45",
              lastLogin: "01-Jan-25 10:30"
            },
            {
              id: 7,
              userId: "usr202407",
              ...
              joinedOn: "21-Dec-24 16:20",
              lastLogin: "31-Dec-24 08:45"
            },
            {
              id: 8,
              userId: "usr202408",
          ...
          page: 1,
          perPage: 10,
          selectedRole: 'All Users', // Default filter

          // Initialize function
          init() {
            console.log('Users table initialized with 8 records');
            // Initialize stats when table loads
            const store = Alpine.store('userModal');
            store.updateStats(this.filteredRows);
          },
          ...
          // Get unique user roles for dropdown
          get userRoles() {
          ...
          // Pagination and table functions
          get totalPages() {
          ...
          getStatusClass(status) {
            switch(status) {
              ...
              case 'De-Activated':
                return 'bg-danger-50 text-danger-700 dark:bg-danger-500/15 dark:text-danger-500';
              case 'Pending':
                return 'bg-info-50 text-info-700 dark:bg-info-500/15 dark:text-info-500';
              case 'Blacklisted':
          ...
          // Update function that reads from the store
          updateUser() {
            const store = Alpine.store('userModal');
        ...
        // 5. The User Roles Table Component - FIXED VERSION
        Alpine.data('userRolesTable', () => ({
          ...
          init() {
            console.log('UserRolesTable init called');
            
            // Get data from global variable
            if (window.GlobalUserRolesData && Array.isArray(window.GlobalUserRolesData)) {
              this.userRoles = window.GlobalUserRolesData;
              console.log('Loaded', this.userRoles.length, 'roles from database');
              
              if (this.userRoles.length > 0) {
                console.log('Sample role data:', this.userRoles[0]);
                // Check what properties are available
                console.log('Available properties:', Object.keys(this.userRoles[0]));
              }
            } else {
              console.warn('No user roles data found in GlobalUserRolesData');
              console.log('GlobalUserRolesData:', window.GlobalUserRolesData);
            ...
            // Debug: Log the computed paginatedRoles
            console.log('paginatedRoles count:', this.paginatedRoles.length);
          },
          ...
          // Pagination and table functions - KEEP AS IS
          get totalPages() {
